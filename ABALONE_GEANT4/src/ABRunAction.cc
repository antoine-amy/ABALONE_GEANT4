...
void ABRunAction::BeginOfRunAction(const G4Run* /*run*/)
{
	...
	auto analysisManager = G4AnalysisManager::Instance();
	// Open an output file
	G4String fileName = "/home/amy/ABALONE/ABALONE_simulation/results/tracking/"+std::to_string(nPrimeries)+sepp+std::to_string((int)theta_source)+sepp+"track_"+std::to_string(file_count);
