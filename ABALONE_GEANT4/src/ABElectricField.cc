...
std::string filename_datar = "/home/amy/ABALONE/ABALONE_simulation/E_fields/x_Efield_25.csv"; //x_Efield_25.csv
std::string filename_dataz = "/home/amy/ABALONE/ABALONE_simulation/E_fields/z_Efield_25.csv"; //z_Efield_25.csv
...
void ABElectricField::GetFieldValue(const G4double point[4],double *eField) const{
	...
	count=count+1;
 	...
 	if (r_particle<=7.0){
		...
		eField[1] = 0.; //By
		eField[2] = 0.; //Bz
		eField[3] = E_x; //eField[3] = E_x; //E_x 
		eField[4] = E_y; //eField[4] = E_y; //E_y
		eField[5] = E_z; //eField[5] = E_z; //E_z
		eField[6] = 0.; //Useless
	}else{
		eField[0] = 0.; //Bx
		eField[1] = 0.; //By
